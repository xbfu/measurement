...
from model import vgg, densenet, mobilenetv2, alexnet, googlenet, resnet3, resnet
    ...
    def update_and_fetch_model(ps_rref, grads, name, i, batch_idx, cm_t0_start, cm_t1_end):
        ...
        with self.lock:
            ...
            if self.curr_update_size >= self.batch_update_size:
                ...
                t = time.time()
                for j in range(len(self.cm_t1_start)):
                    self.cm_t1_start[j] = t
...
def get_accuracy(ps_rref, data_dir, test_batch_size, job_name, target_accuracy):
...
def run(rank, num_workers, data_dir, model, batch_size, test_batch_size, lr, num_epochs, job_name, target_accuracy):
    ...
    if rank == 0:
        ...
        futs.append(rpc.rpc_async(to="tester",
                                  func=get_accuracy,
                                  args=(ps_rref, data_dir, test_batch_size, job_name, target_accuracy)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train models on CIFAR under ASGD")
    ...
    run(args.rank, args.num_workers, args.data_dir, args.model, args.batch_size, args.test_batch_size, args.lr,
        args.num_epochs, args.job_name, args.target)